...
4. Slurmでの実行: Slurmを利用する場合は, `main.sh`に必要なジョブスクリプトを記述し, `sbatch main.sh`で実行する。出力は全て`results/`に保存されるようにする。パスはmain.sh内でコマンドラインからargparseなどを利用してpythonスクリプトに渡す形で管理する。
