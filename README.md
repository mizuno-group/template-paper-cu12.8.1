...
## 特徴
  ...
  - **Slurm対応**: Slurmでの実行を想定したスクリプト (`main.sh`, `setup_venv.sh`) を提供し, 大規模な実験も再現可能にする。
...
├── .gitignore
├── LICENSE
├── env.def          # 最低限の環境情報をまとめたもの
├── pyproject.toml   # 依存関係とプロジェクト設定
├── main.sh          # Slurmでの実行スクリプト
...
1.  データ準備: 論文で用いる生データを```data```に配置する。必要であれば, ダウンロード方法を```README.md```に記述する。Slurm利用の場合、転送速度によってはdata.tar.gzとして圧縮しておく。
...
4. Slurmでの実行: Slurmを利用する場合は, `main.sh`に必要なジョブスクリプトを記述し, `sbatch main.sh`で実行する。環境構築も同様に`setup_venv.sh`を利用して行う。出力は全て`results/`に保存されるようにする。パスはmain.sh内でコマンドラインからargparseなどを利用してpythonスクリプトに渡す形で管理する。
5.  図の作成: ```notebooks/```で, ```results/```に保存された実験結果を読み込み, 論文に掲載する図を作成・保存する。3の過程で出力されるならその旨を記し, 全てのFigureの出所がここからわかるようにする。
6.  README.mdの編集: 下記の英語テンプレートを編集し, 第三者が迷わずに結果を再現できるよう, 具体的な手順を記述する。
